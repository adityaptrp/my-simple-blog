...
@section('form_content')
    ...
    <h4 class="mt-4 text-xl text-gray-800">Welcome to <span class="font-bold">{{ config('app.name') }}</span></h4>
    ...
    <form action="{{ route('register') }}" method="POST" class="mt-5" autocomplete="off">
        @csrf
        @method('post')
        @if (session('success'))
        ...
        @endif
        <div class="relative @error('name') error-input @enderror">
            <label class="label-auth" for="name">Name :</label>
            <input class="input-auth" type="text" name="name" id="name" placeholder="Input name" value="{{ old('name') ?? '' }}">
        </div>
        <div class="relative mb-3 @error('username') error-input @enderror">
            <label class="label-auth" for="username">Username :</label>
            <input class="input-auth border-t-0" type="text" name="username" id="username" placeholder="Input username" value="{{ old('username') ?? '' }}">
        </div>

        <div class="relative @error('email') error-input @enderror">
            <label class="label-auth" for="email">Email :</label>
            <input class="input-auth" type="text" name="email" id="email" placeholder="Input email" value="{{ old('email') ?? '' }}">
        </div>
        <div class="flex mb-3">
        ...
        <div>
            <label class="inline-flex items-center mt-2 @if($errors->any()) mb-2 @endif cursor-pointer">
                <input type="checkbox" class="cursor-pointer -mt-5" name="terms_of_service">
                <div class="ml-2 text-gray-700 text-sm cursor-pointer">
                    I agree to the 
        ...
        <div class="mt-5 flex justify-end items-center">
            <button class="btn btn-indigo" type="submit" name= "submit">Register</button>
        </div>
    </form>
    ...
    <div class="flex items-center justify-center mt-8 text-sm">
        ...
        <a href="{{ route('login') }}" class="ml-1 text-blue-500 font-semibold">Login</a>
    </div>
