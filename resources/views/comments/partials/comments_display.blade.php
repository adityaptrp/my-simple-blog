...
@foreach ($comments as $i => $comment)
    @if ($comment->is_approved == 1)
        <div class="list-comment mt-5 @if($comment->parent_id != null) replies-comment @if($comments->where('is_approved', 1)->last()->id!=$comment->id) pb-5 @endif @if($comments->where('is_approved', 1)->last()->id==$comment->id) noborder-b @endif ml-5 @endif" data-parent="{{ $comment->parent_id }}" data-comment-id="{{ $comment->id }}">
            {{-- profile --}}
            <div class="flex @if (Auth::user() || Cookie::get('comment_submitted') == $comment->id) justify-between @endif">
                <div class="flex items-center">
                    ...
                    <div class="txt-profile-comment flex flex-col text-sm ml-2">
                        <div class="flex items-center">
                            <a class="font-bold {{ $comment->user_link ? 'text-green-500' : '' }}" @if ($comment->user_link) href="{{ $comment->user_link }}" target="blank" @endif @if (Auth::user()) title="{{ $comment->email }}" @endif>{{ $comment->name }}</a>
                            ...
                            @else
                                @auth
                                    ...
                                    @endif
                                @else
                                    @if (Cookie::get('comment_submitted') == $comment->id)
                                        <span class="status ml-1 you rounded-sm">You</span>
                                    @endif
                                @endauth
                            @endif
                ...
                @auth
                    @if ($comment->post->user->id == Auth::user()->id || Auth::user()->email == $comment->email)
                        <div class="dd-delete text-lg cursor-pointer">
                            <i class="fas fa-ellipsis-h"></i>
                            <div>
                                <p class="btn-delete hover:text-red-500 hover:bg-blue-100" data-comment-id="{{ Crypt::encryptString($comment->id) }}">Delete</p>
                            </div>
                        </div>
                    @endif
                @else
            ...
            {{-- body --}}
            @if (Cookie::get('comment_submitted') == $comment->id)
                <div class="message-unapproved mt-4 -mb-2 text-sm text-gray-500 italic">
                    <p>This is your comment, you can still delete this comment until one day later or until you make another comment.</p>
                </div>
            @endif
            {{-- selected text --}}
            @if ($comment->selected_text)
                <div class="selected-txt-cmt show-text-selected border mt-5 mb-3 py-3 px-3.5 rounded">
                    <span class="text-base">{{ $comment->selected_text }}</span>
                    <input type="hidden" name="selected_text" value="">
                </div>
            @endif
            <div class="text-b_base mt-4 overflow-auto">
                <?php 
                ...
                ?>
                <p>{!! Str::limit(nl2br($body), 400, '...') !!}</p>
                @if ( Str::length($body) > 400 )
                    <p class="btn-readmore-comment font-semibold text-sm mt-2 text-green-600 cursor-pointer">Read More</p>
                @endif
            </div>
            {{-- footer --}}
            <div class="cmnt-footer mt-4 flex items-center relative">
                {{-- like --}}
                @if (isset($dataCookie))
                ...
                @if (in_array($comment->id, $arr))
                    <a class="cmnt-clapped flex items-center cursor-pointer h-full">
                        <svg class="w-3.5 h-3.5 md:w-4.5 md:h-4.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 285.25 284.04" fill="salmon">
                ...
                @endif
                {{-- reply --}}
                <a class="show-form-reply reset-reply-txt flex items-center ml-3 text-sm cursor-pointer">
            ...
            {{-- Form Reply --}}
            <form class="form-reply-comment hidden mt-4 text-sm" autocomplete="off">
                @csrf
            ...
            @auth
                @if (sizeOf($comment->replies->where('is_approved', 1)) > 0)
                    <div class="show-replies-comment inline-block font-semibold text-0.5sm text-blue-500 mt-5 cursor-pointer">
                        <span>Show {{sizeOf($comment->replies->where('is_approved', 1))}} more {{ sizeOf($comment->replies->where('is_approved', 1)) > 1 ? 'replies' : 'reply' }}</span>
                        <div class="el-tambahan"></div>
                    </div>
                @endif
            @else
                @if (sizeOf($comment->replies->where('is_approved', 1)) > 0 || $comment->replies->where('id', Cookie::get('comment_submitted'))->count() != 0)
                    <div class="show-replies-comment inline-block font-semibold text-0.5sm text-blue-500 mt-5 cursor-pointer">
                        <span>Show {{sizeOf($comment->replies->where('is_approved', 1))}} more {{ sizeOf($comment->replies->where('is_approved', 1)) > 1 ? 'replies' : 'reply' }}</span>
                        <div class="el-tambahan"></div>
                    </div>
            ...
            <div class="all-cmnt-replies hidden">
                @include('comments.partials.comments_display', ['comments' => $comment->replies])
            </div>
    ...
    @if (isset($viewCU))
        @if ($comment->id == $viewCU)
            <div class="view-CUnapproved py-5 -mx-6 px-6 @if($comment->parent_id != null) replies-comment-parent mt-5 -mr-6 @endif" data-comment-id="{{ $comment->id }}">
                {{-- profile --}}
                <div class="flex justify-between">
                    ...
                    @if ($comment->post->user->id == Auth::user()->id)
                        <div class="dd-delete text-lg cursor-pointer">
                            <i class="fas fa-ellipsis-h"></i>
                            <div>
                                <p class="btn-approve hover:text-green-500 text-center" data-comment-id="{{ Crypt::encryptString($comment->id) }}">Approve</p>
                                @if ($comment->user_link)
                                    <p class="btn-approve-cwl hover:text-yellow-500 text-center w-32" data-comment-id="{{ Crypt::encryptString($comment->id) }}">Without Link</p>
                                @endif
                                <p class="btn-delete hover:text-red-500 text-center" data-comment-id="{{ Crypt::encryptString($comment->id) }}">Delete</p>
                            </div>
                        </div>
                    @endif
                </div>
                ...
                <div class="text-b_base mt-4">
                    ...
                    <p>{!! Str::limit(nl2br($body), 400, '...') !!}</p>
                    @if ( Str::length($body) > 400 )
                        <p class="btn-readmore-comment text-sm mt-2 text-gr font-semiboldeen-60smcursor-pointer">Read More</p>
                    @endif
                </div>
                {{-- footer --}}
                <div class="cmnt-footer mt-4 flex items-center relative">
    ...
    @if ($comment->is_approved == 0)
        @if (Cookie::get('comment_submitted') == $comment->id)
            @if (!Auth::user())
                <div class="list-comment-new py-5 -mx-6 px-6 @if($comment->parent_id != null) replies-comment-parent mt-5 -mr-6 @endif" data-comment-id="{{ $comment->id }}">
                    {{-- profile --}}
                    <div class="flex justify-between">
                        <div class="flex items-center">
                            <a @if ($comment->user_link) href="{{ $comment->user_link }}" target="blank" @endif class="relative inline-block">
